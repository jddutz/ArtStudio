...
ArtStudio/
├── src/
│   ├── ArtStudio.WPF/           # Main WPF application with MaterialDesign UI
│   │   ├── Views/               # User controls for UI components
│   │   ├── ViewModels/          # MVVM ViewModels
│   │   ├── Services/            # WPF-specific services (DockingService, WindowManager)
│   │   ├── Themes/              # Custom MaterialDesign themes
│   │   ├── App.xaml/cs          # Application entry point with theming
│   │   └── MainWindow.xaml/cs   # Main application window with AvalonDock
│   ├── ArtStudio.Core/          # Core business logic and interfaces
│   │   ├── Interfaces/          # Service interfaces (ILayoutManager, IEditorService)
│   │   └── Services/            # Core service implementations
│   ├── ArtStudio.UI/            # Shared UI components (currently minimal)
│   └── ArtStudio.AI/            # AI integration modules (future implementation)
├── tests/
│   └── ArtStudio.Tests/         # Unit tests using xUnit
├── .github/
│   └── workflows/               # CI/CD pipeline definitions
├── docs/                        # Documentation files
├── run.bat                      # Convenience script for quick development runs
├── publish.bat                  # Convenience script for publishing releases
└── .vscode/tasks.json          # VS Code task definitions for building
```
...
### GitHub Copilot Work Area

- **Work Directory**: Use `.github\agent\` as the designated work area for all temporary files, work plans, checklists, task instructions, scripts, and project management documents
- **File Organization**: 
  - Create subdirectories as needed (e.g., `.github\agent\tasks\`, `.github\agent\plans\`, `.github\agent\scripts\`)
  - Use descriptive filenames with dates when appropriate
  - Clean up temporary files after task completion when appropriate
- **User Awareness Protocol**: 
  - **MANDATORY**: Before starting any new task or significant work, explicitly confirm that you have read and understood these instructions
  - State which sections of the instructions are relevant to the current task
  - Reference any previous work files from `.github\agent\` if continuing from earlier sessions
  - Ask for clarification if any part of the task conflicts with these established guidelines

### Code Quality & Warning Management

- **ALL warnings MUST be treated as high-priority issues** that require immediate resolution
- The project is configured with `TreatWarningsAsErrors=true` - any warning will break the build
- Before any code changes are submitted, ensure the build completes with 0 warnings and 0 errors

#### Warning Suppression Guidelines

When suppressing warnings using `#pragma warning disable`:

1. **ALWAYS include a detailed comment** explaining:
   - Why the warning is being suppressed
   - What specific scenario makes the warning inappropriate
   - When it might be safe to remove the suppression
   - Any related issue numbers or documentation references

2. **Example of proper warning suppression**:
   ```csharp
   #pragma warning disable CS1591 // Missing XML comment for publicly visible type
   // TODO: Remove this suppression after implementing comprehensive XML documentation
   // Tracked in issue #123 - XML documentation will be added in the next sprint
   public class TemporaryApiClass
   {
       // implementation
   }
...
3. **Use `#pragma warning restore`** immediately after the code that needs the suppression
4. **Avoid global warning suppressions** - prefer local suppressions with clear justification

#### Warning Resolution Priority
1. **Fix the underlying issue** whenever possible (preferred approach)
2. **Refactor code** to eliminate the warning condition
3. **Suppress only as a last resort** with proper justification and future remediation plan

### Code Organization

- All classes, interfaces, types, etc. shall be defined in their own file
